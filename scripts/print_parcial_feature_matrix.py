...
##
# usage: python print_partial_evidence_type busqueda_id personid_min_limit personid_max_limit
...
for p in busqueda.persona_set.all():
    if min_limit <= p.id and p.id <= max_limit: 
        logging.debug("processing person " +p.name+ " with id=" +str(p.id))
        for s in p.snippet_set.filter(FG=1).exclude(RE=1):
            if (s.evidence_type is None):  
                evidence_type = '0'
            elif s.evidence_type == '3':
                evidence_type = '2'
            else:
