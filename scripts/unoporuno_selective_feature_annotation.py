...
for p in busqueda.persona_set.all():
    ...
    for s in p.snippet_set.all():
        ...
        countries = CountryGazet.typed_list_test(descr_test_str) if selection == 'co' else []
        if len(countries)>0:
            logging.debug('FOUND COUNTRIES '+str(countries)+' IN DESCR:\n ' +s.description)
            s.RE_features |= 2
            snippet_countries += countries
        #todo: buscar países en la address ip y la nacionalidad
        countries = CountryGazetCase.typed_list_test(title_test_str) if selection == 'co' else []
        if len(countries)>0:
            logging.debug ('FOUND COUNTRIES '+str(countries)+' IN TITLE:\n ' +s.title)
            s.RE_features |= 2
            snippet_countries += countries
        countries = CountryGazetCase.typed_list_test(descr_test_str) if selection == 'co' else []
        if len(countries):
            logging.debug('FOUND COUNTRIES '+str(countries)+' IN DESCR:\n ' +s.description)
            s.RE_features |= 2
            snippet_countries += countries
        countries = CityGazet.typed_list_test(title_test_str) if selection == 'ci' else []
        if len(countries)>0:
            logging.debug('FOUND CITIES FROM COUNTRIES '+str(countries)+' IN CI TITLE\n' +s.title)
            s.RE_features |= 4
            snippet_countries += countries
        countries = CityGazet.typed_list_test(descr_test_str) if selection == 'ci' else []
        if len(countries)>0:
            logging.debug('FOUND CITIES FROM COUNTRIES '+str(countries)+' IN CI DESCRIPTION\n' +s.description)
            s.RE_features |= 4
            snippet_countries += countries
        countries = CityGazetCase.typed_list_test(title_test_str) if selection == 'ci' else []
        if len(countries)>0:
            logging.debug('FOUND CITIES FROM COUNTRIES '+str(countries)+' IN CD TITLE\n' +s.title)
            s.RE_features |= 4
            snippet_countries += countries        
        countries = CityGazetCase.typed_list_test(descr_test_str) if selection == 'ci' else []
        if len(countries)>0:
